...
const simulateAIChatResponse = async (
    word: string | null,
    context: string | null,
    history: ChatMessage[],
    newMessage: string,
    ...
    const lastUserMessage = newMessage.toLowerCase();

    if (lastUserMessage.includes('example') || lastUserMessage.includes('sentence')) {
        return `Sure, here's an example sentence for **${word}**: "The quick brown **${word}** jumps over the lazy dog."`;
    }
    if (lastUserMessage.includes('synonym')) {
        return `A common synonym for **${word}** could be 'equivalent' or 'parallel', depending on the exact nuance you're looking for.`;
    }
    if (lastUserMessage.includes('antonym')) {
        return `An antonym for **${word}** might be 'different' or 'opposite'.`;
    }
    if (lastUserMessage.includes('etymology') || lastUserMessage.includes('origin')) {
        return `The etymology of **${word}** is quite interesting, often tracing back to Latin or Old English roots. I can look that up if you'd like more specifics!`;
    }
    return `I'm here to help you discuss **${word}**. You asked: "${newMessage}". What else would you like to explore about it in the context of "${context || 'its general meaning'}"?`;
};
